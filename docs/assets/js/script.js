/* ═══════════════════════════════════════════════════════
   PRINT X — Global JavaScript Utilities v2.0
   Fixes: renderFooter auto-call, hamburger state, admin token,
...
async function updateCartCount() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const customItems = res.data?.custom_requests || [];
      const productCount = cartItems.reduce((s, item) => s + (item.quantity || 1), 0);
      const customCount = customItems.length;
      const totalCount = productCount + customCount;
...
function statusBadge(status) {
  const map = {
    ...
    cancelled: 'error', reviewing: 'info',
  };
